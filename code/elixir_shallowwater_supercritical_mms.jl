...
@inline function initial_condition_moving_blob(x, t, equations::ShallowWaterEquations2D)
    ...
    # reference water height (supercritical)
...
function source_term_moving_blob(u, x, t, equations::ShallowWaterEquations2D)
    # Parameters for the manufactured solution
    # Note, must match those from the `initial_condition_moving_blob`
...
# Outflow boundary condition for the shallow water equations where
# all solution information should come from the internal solution.
@inline function boundary_condition_supercritical_outflow(u_inner, normal_direction::AbstractVector,
...
@inline function boundary_condition_supercritical_outflow_llf(u_inner, normal_direction::AbstractVector,
                                                              x, t, surface_flux_functions,
                                                              equations::ShallowWaterEquations2D)
    _, nonconservative_flux_function = surface_flux_functions
    # Calculate the boundary flux entirely from the external solution state.
    u_boundary = initial_condition_moving_blob(x, t, equations)
...
@inline function boundary_condition_supercritical_outflow_hll(u_inner, normal_direction::AbstractVector,
                                                              x, t, surface_flux_functions,
                                                              equations::ShallowWaterEquations2D)
    _, nonconservative_flux_function = surface_flux_functions
    # Calculate the boundary flux entirely from the external solution state.
...
# Inflow boundary condition for the shallow water equations where
# all solution information should come from the external solution.
@inline function boundary_condition_supercritical_inflow_llf(u_inner, normal_direction::AbstractVector,
                                                             x, t, surface_flux_functions,
                                                             equations::ShallowWaterEquations2D)
    _, nonconservative_flux_function = surface_flux_functions
    # Calculate the boundary flux entirely from the external solution state.
...
@inline function boundary_condition_supercritical_inflow_hll(u_inner, normal_direction::AbstractVector,
                                                             x, t, surface_flux_functions,
                                                             equations::ShallowWaterEquations2D)
    _, nonconservative_flux_function = surface_flux_functions
    # Calculate the boundary flux entirely from the external solution state.
...
###############################################################################
# Novel nonlinearly stable subcritical inflow boundary fluxes
#   - 3 BCs at inflow and 0 BC at outflow when abs(un) > c
@inline function boundary_condition_supercritical_inflow_stable(u_inner, normal_direction::AbstractVector,
                                                                x, t, surface_flux_functions,
                                                                equations::ShallowWaterEquations2D)
    _, nonconservative_flux_function = surface_flux_functions
    ...
    lambda3_geom = sqrt((abs(u_n) - c) * (abs(u_n_ext) - c_ext))

    # Compute numerical flux components
    f1 = ((alfa - 1) * h * u_n
          - lambda1_geom * alfa * c_ext * (alfa * c_ext - u_n_ext) * s2g
          - lambda3_geom * alfa * c_ext * (alfa * c_ext + u_n_ext) * s2g)

    f2 = ((0.5 * alfa - 1) * h * u * u_n + 0.5 * (1 - 2 * alfa) * g * h^2 * normal[1]
          - lambda1_geom * c_ext * (alfa * c_ext - u_n_ext) * (alfa * u - 2 * c * normal[1]) * s4g
          + lambda2_geom * h_geom * u_t_ext * normal[2]
          - lambda3_geom * c_ext * (alfa * c_ext + u_n_ext) * (alfa * u + 2 * c * normal[1]) * s4g)

    f3 = ((0.5 * alfa - 1) * h * v * u_n + 0.5 * (1 - 2 * alfa) * g * h^2 * normal[2]
          - lambda1_geom * c_ext * (alfa * c_ext - u_n_ext) * (alfa * v - 2 * c * normal[2]) * s4g
          - lambda2_geom * h_geom * u_t_ext * normal[1]
          - lambda3_geom * c_ext * (alfa * c_ext + u_n_ext) * (alfa * v + 2 * c * normal[2]) * s4g)
...
boundary_condition = Dict(:Right  => boundary_condition_slip_wall,
                          ...
                          :Bottom => boundary_condition_supercritical_inflow_stable,
                          :Top    => boundary_condition_supercritical_outflow)
